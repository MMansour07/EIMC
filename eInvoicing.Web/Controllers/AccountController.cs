...
namespace eInvoicing.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Register()
        {
            ...
            new SelectListItem { Value = rr.Id, Text = rr.Name }).ToList();
            ViewBag.Roles = rolelist;
            //ViewBag.RolesList = _roleService.GetAllIncluding().Select(i => new { Value = i.Id, Text = i.Name }).ToList();
            return View();
        }
        ...
        public async Task<ActionResult> Register(AccountVM model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.RegisterViewModel.Email.Split('@')[0], Email = model.RegisterViewModel.Email, FirstName = model.RegisterViewModel.FullName[0].ToString(), LastName = model.RegisterViewModel.FullName[1].ToString() };
                ...
                if (result.Succeeded)
                {
                    ...
                    return View("~/Views/Account/CheckEmail.cshtml");
