...
namespace eInvoicing.API.Filters
{
    public class HMACAuthenticationAttribute : Attribute, IAuthenticationFilter
    {
        ...
        public HMACAuthenticationAttribute(IConfiguration configuration)
        {
            _configuration = configuration;
            if (allowedApps.Count == 0)
            {
                allowedApps.Add(_configuration.GetSection("AppSettings").GetSection("AppId").Value, _configuration.GetSection("AppSettings").GetSection("AppKey").Value);
            }
        }
